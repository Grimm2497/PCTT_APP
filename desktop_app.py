...
class DesktopApp(tk.Tk):
    ...
    def _on_provider_changed(self, event=None):
        ...
        if provider in PROVIDER_PRESETS:
            ...
            if not self.base_url_var.get().strip() and preset.get("base_url"):
    ...
    def _build_ui(self):
        ...
        ttk.Entry(cfg, textvariable=self.model_var).grid(row=2, column=1, sticky="ew", pady=(8, 0))
        ...
        ttk.Entry(cfg, textvariable=self.api_key_var, show="*").grid(row=4, column=1, sticky="ew", pady=(8, 0))
        ttk.Label(cfg, text="Tự dừng sau (giây):").grid(row=5, column=0, sticky="w", pady=(8, 0))
        ttk.Entry(cfg, textvariable=self.timeout_var).grid(row=5, column=1, sticky="ew", pady=(8, 0))
    ...
    def _run_analysis_worker(self):
        try:
            ...
            det_result = analyze_structured_files(self.input_files)
    ...
    def _show_error(self, err: str, detail: str):
        ...
        self.stop_button.configure(state=tk.DISABLED)
        self.json_text.delete("1.0", tk.END)
        self.json_text.insert(tk.END, detail)
